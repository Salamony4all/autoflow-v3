...
class WIRGenerator:
    """Generate Work Inspection Request (WIR) with Alshaya branded template"""
    ...
    def generate(self, file_id, session, project_settings=None):
        ...
        output_dir = os.path.join('outputs', session_id, 'wir')
        os.makedirs(output_dir, exist_ok=True)

        output_file = os.path.join(output_dir, f'wir_{file_id}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.pdf')
        ...
        for idx, item in enumerate(items):
            ...
            story.extend(self._create_wir_page(item, idx + 1, len(items)))
    ...
    def _create_wir_page(self, item, item_num, total_items):
        """Create a branded WIR page for one item"""
        story = []
        gold = colors.HexColor(self.GOLD)
        dark = colors.HexColor(self.DARK_NAVY)
        ...
        # Title
        header_data = [
            ['WORK INSPECTION REQUEST', f'Item {item_num} of {total_items}']
        ]
        ...
        # Project Info - use project settings if provided
        ps = getattr(self, 'project_settings', {}) or {}
        project_data = [
            ['Project:', ps.get('project_name', '[Project Name]'), 'WIR No:', f'WIR-{str(item_num).zfill(3)}'],
            ['Contractor:', ps.get('contractor', 'Al Shaya Enterprises'), 'Date:', datetime.now().strftime('%d/%m/%Y')],
            ['Consultant:', ps.get('consultant', '[Consultant Name]'), 'Location:', ps.get('location', '[Location/Zone]')],
            ['Activity:', '[Installation/Fixing]', 'Discipline:', 'FF&E / Joinery'],
        ]
        project_table = Table(project_data, colWidths=[1.0 * inch, 3.0 * inch, 0.9 * inch, 2.1 * inch])
        ...
        work_data = [
            ['Item Description:', Paragraph(description_text, desc_style)],
            ['Brand/Manufacturer:', item.get('brand', 'To be specified')],
            ['Quantity:', f"{item.get('qty', 'N/A')} {item.get('unit', '')}"],
            ['Specification Ref:', f'As per approved MAS-{str(item_num).zfill(3)}'],
            ['Drawing Ref:', ps.get('drawing_ref', '[Drawing Reference Number]')],
        ]
        work_table = Table(work_data, colWidths=[1.5 * inch, 5.5 * inch])
        ...
        inspection_data = [
            ...
            ['☐  During installation inspection (work in progress)'],
            ['☐  Final inspection (completed work)'],
            ['☐  Mock-up / Sample approval'],
        ]
        inspection_table = Table(inspection_data, colWidths=[7 * inch])
        ...
        response_data = [
            ...
            ['☐  APPROVED WITH COMMENTS — Work may proceed subject to comments'],
            ['☐  NOT APPROVED — Work does not comply, remedial action required'],
            ['☐  HOLD — Further information/documentation required'],
        ]
        response_table = Table(response_data, colWidths=[7 * inch])
        ...
        for img_path in image_paths[:2]:
            ...
            if img_path and os.path.exists(img_path):
                try:
                    ...
                    logger.warning(f"Failed to add image to WIR: {e}")
